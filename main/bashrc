...
alias open="xdg-open"
alias tree2="tree -I '*__pycache__|venv'"
...
function newassignment() {
    cp ~/latex/assignmentTemplate.tex "$1.tex"
    nvim "$1.tex"
}

getnews () {
     curl https://newsapi.org/v2/top-headlines -s -G -d sources=$1 -d category=general -d apiKey=ENTER_KEY_HERE | jq -r '.articles[] | .title, .url, ""'
}
...
# some stuff for installing Ruby Gems used for Jekyll
